package com.festcube.openfire.plugin.xep0059;
...
public class XmppResultSet
{
    ...
    public XmppResultSet(Element setElement)
    {
        if (setElement.element("after") != null)
        {
            try
            {
                after = setElement.elementText("after").trim().replace("\n", "");
            }
            catch (Exception e)
            {
                // swallow
        ...
        if (setElement.element("before") != null)
        {
            try
            {
                before = setElement.elementText("before").trim().replace("\n", "");
            }
            catch (Exception e)
            {
                // swallow
        ...
        if (setElement.element("max") != null)
        {
            try
            {
                max = Long.parseLong(setElement.elementText("max").trim().replace("\n", ""));
                if (max < 0)
                {
                    max = null;
                }
        ...
        if (setElement.element("index") != null)
        {
            try
            {
                index = Long.parseLong(setElement.elementText("index").trim().replace("\n", ""));
                if (index < 0)
                {
                    index = null;
                }
